...
# OCC/pybind11 可能因暫態錯誤而初始化失敗，加入重試機制
import time as _time_mod
CADQUERY_AVAILABLE = False
for _cq_attempt in range(3):
    try:
        ...
        if _cq_attempt < 2:
            logger.warning(f"CadQuery import attempt {_cq_attempt + 1}/3 failed: {e}, retrying...")
            _time_mod.sleep(1)
        else:
            logger.warning(f"CadQuery not available after 3 attempts: {e}")
...
class MockCADEngine:
    ...
    def _load_drafter_config(self, model_file: str):
        ...
        有效的 key（全部可選）：
          ...
          show_basic_info, show_leg_angles, show_track_relations
        """
    ...
    def _build_tb_info(self, info: Dict, base_name: str,
        ...
        tb = {
            ...
            'version':        defaults['version'],   # 程式版本，不從 config 覆蓋
    ...
    def _analyze_bspline_pipe(self, solid_shape, feature_id, params):
        ...
        # === 端點校正：3D 圓擬合取得管軸心 ===
        # BSpline 取樣在 sv_mid 的端點是管壁上的點，不是管中心
        # V 參數僅涵蓋半圓周，簡單平均會有偏差
        # 改用 3D 圓擬合（在截面平面上擬合圓）取得精確管軸心
        corrected_start = None
        corrected_end = None
        _arc_length_from_u = 0
        try:
            ...
            _arc_length_from_u = _su2 - _su1  # U 參數範圍 = 弧長（精確）
            n_v = 36  # 沿 V 方向取 36 個點

            def _fit_pipe_center(u_val):
                """在 u_val 處取 V 截面點，擬合 3D 圓取得管軸心"""
                pts_c = []
                for j in range(n_v):
                ...
                nc = len(pts_c)
                # 質心
                mx = sum(p[0] for p in pts_c) / nc
                my = sum(p[1] for p in pts_c) / nc
                mz = sum(p[2] for p in pts_c) / nc
                # 中心化
                cen = [(p[0]-mx, p[1]-my, p[2]-mz) for p in pts_c]
                # 截面平面法向量（用跨度向量叉積）
                v1 = cen[nc // 3]
                v2 = cen[2 * nc // 3]
                ...
                nl = math.sqrt(nx_**2 + ny_**2 + nz_**2)
                if nl < 1e-10:
                    return (mx, my, mz)
                nx_, ny_, nz_ = nx_/nl, ny_/nl, nz_/nl
                # 局部 2D 座標系
                p0 = cen[0]
                dn = p0[0]*nx_ + p0[1]*ny_ + p0[2]*nz_
                e1x = p0[0] - dn*nx_
                e1y = p0[1] - dn*ny_
                e1z = p0[2] - dn*nz_
                e1l = math.sqrt(e1x**2 + e1y**2 + e1z**2)
                if e1l < 1e-10:
                    return (mx, my, mz)
                ...
                e2z = nx_*e1y - ny_*e1x
                # 投影到 2D
                pts2 = [(c[0]*e1x+c[1]*e1y+c[2]*e1z,
                         c[0]*e2x+c[1]*e2y+c[2]*e2z) for c in cen]
                # Kasa 最小平方圓擬合
                np2 = len(pts2)
                sx_ = sum(q[0] for q in pts2)
                sy_ = sum(q[1] for q in pts2)
                sx2_ = sum(q[0]**2 for q in pts2)
                sy2_ = sum(q[1]**2 for q in pts2)
                sxy_ = sum(q[0]*q[1] for q in pts2)
                sf_ = [q[0]**2+q[1]**2 for q in pts2]
                sfx_ = sum(sf_[k]*pts2[k][0] for k in range(np2))
                sfy_ = sum(sf_[k]*pts2[k][1] for k in range(np2))
                sfs_ = sum(sf_)
                A11_, A12_, A13_ = sx2_, sxy_, sx_
                A21_, A22_, A23_ = sxy_, sy2_, sy_
                A31_, A32_, A33_ = sx_, sy_, float(np2)
                det_ = (A11_*(A22_*A33_-A23_*A32_)
                        - A12_*(A21_*A33_-A23_*A31_)
                        + A13_*(A21_*A32_-A22_*A31_))
                if abs(det_) < 1e-10:
                    return (mx, my, mz)
                av_ = ((sfx_*(A22_*A33_-A23_*A32_)
                        - A12_*(sfy_*A33_-A23_*sfs_)
                        + A13_*(sfy_*A32_-A22_*sfs_)) / det_)
                bv_ = ((A11_*(sfy_*A33_-A23_*sfs_)
                        - sfx_*(A21_*A33_-A23_*A31_)
                        + A13_*(A21_*sfs_-sfy_*A31_)) / det_)
                cu_ = av_ / 2
                cv_ = bv_ / 2
                # 轉回 3D
                return (mx + cu_*e1x + cv_*e2x,
                        my + cu_*e1y + cv_*e2y,
                        mz + cu_*e1z + cv_*e2z)

            corrected_start = _fit_pipe_center(_su1)
            corrected_end = _fit_pipe_center(_su2)
        except Exception:
            _arc_length_from_u = 0
            pass
        ...
        if straightness > 0.99:
            ...
            # 彎管 — 使用 3D 圓擬合取得精確管軸心中心線
            # 在每個 U 取樣點，擬合截面圓找管心（比 sv_mid 壁面取樣更精確）
            true_centerline = None
            try:
                if corrected_start is not None:
                    # _fit_pipe_center 已定義，用它取樣 TRUE 中心線
                    true_centerline = []
                    for i in range(n_samples + 1):
                        u_val = _su1 + i / n_samples * (_su2 - _su1)
                        true_centerline.append(_fit_pipe_center(u_val))
            except Exception:
                true_centerline = None

            # 用於擬合的中心線（優先 true_centerline）
            fit_cl = true_centerline if true_centerline else centerline

            # 嘗試 3 個投影平面（XY, XZ, YZ），選殘差最小的
            best_arc = None
            best_residual = float('inf')
            axis_combos = [(0, 1, 2), (0, 2, 1), (1, 2, 0)]  # (ax0, ax1, h_ax)

            for ax0, ax1, h_ax in axis_combos:
                pts_2d = [(p[ax0], p[ax1]) for p in fit_cl]
                n_pts = len(pts_2d)

                # Kasa 最小平方圓擬合
                sx  = sum(p[0] for p in pts_2d)
                sy  = sum(p[1] for p in pts_2d)
                sx2 = sum(p[0]**2 for p in pts_2d)
                sy2 = sum(p[1]**2 for p in pts_2d)
                sxy = sum(p[0]*p[1] for p in pts_2d)
                sf  = [p[0]**2 + p[1]**2 for p in pts_2d]
                sfx = sum(sf[i] * pts_2d[i][0] for i in range(n_pts))
                sfy = sum(sf[i] * pts_2d[i][1] for i in range(n_pts))
                sf_sum = sum(sf)

                A11, A12, A13 = sx2, sxy, sx
                A21, A22, A23 = sxy, sy2, sy
                A31, A32, A33 = sx, sy, float(n_pts)
                ...
                a_val = ((sfx*(A22*A33 - A23*A32)
                          - A12*(sfy*A33 - A23*sf_sum)
                          + A13*(sfy*A32 - A22*sf_sum)) / D)
                b_val = ((A11*(sfy*A33 - A23*sf_sum)
                          - sfx*(A21*A33 - A23*A31)
                          + A13*(A21*sf_sum - sfy*A31)) / D)

                ux = a_val / 2
                uy = b_val / 2

                dists = [math.sqrt((p[0] - ux)**2 + (p[1] - uy)**2)
                         for p in pts_2d]
                r = sum(dists) / len(dists)
                std_d = math.sqrt(sum((d - r)**2 for d in dists) / len(dists))
                residual = std_d / r if r > 0 else float('inf')

                if residual < best_residual:
                    # 累積角度
                    total_ac = 0.0
                    for i in range(len(pts_2d) - 1):
                        a1_ = math.atan2(pts_2d[i][1] - uy, pts_2d[i][0] - ux)
                        a2_ = math.atan2(pts_2d[i+1][1] - uy, pts_2d[i+1][0] - ux)
                        da = a2_ - a1_
                        while da > math.pi:
                            da -= 2 * math.pi
                        while da < -math.pi:
                    ...
                    if corrected_start and corrected_end:
                        hg = abs(corrected_end[h_ax] - corrected_start[h_ax])
                    else:
                        hg = abs(fit_cl[-1][h_ax] - fit_cl[0][h_ax])

                    best_residual = residual
                    best_arc = {
                        'radius': r,
                        'angle': abs(total_ac),
                        'angle_deg': round(math.degrees(abs(total_ac)), 1),
                        'height_gain': hg,
                        'height_axis': h_ax,
                    }

            if best_arc and best_arc['angle_deg'] > 5:
                arc_radius = best_arc['radius']
                arc_angle = best_arc['angle']
                arc_angle_deg = best_arc['angle_deg']
                height_gain = best_arc['height_gain']
                height_axis = best_arc['height_axis']

                # 弧長：優先使用 U 參數範圍（最精確）
                if _arc_length_from_u > 0:
                    cl_arc = _arc_length_from_u
                elif height_gain > 1:
    ...
    def _generate_cutting_list(self) -> Dict:
        ...
        for i, leg in enumerate(leg_parts, 1):
            ...
            # total_length = (bbox_max + face_extent) / 2
            #   bbox_max = 實體邊界框最大維度（含底板等附屬結構）
            #   face_extent = 圓柱面沿管方向跨距（較接近純管長）
            #   平均值排除底板等多餘幾何的影響，得到最準確的管件物理長度
            if feat:
                bl = feat.params.get('bbox_l', 0)
                bw = feat.params.get('bbox_w', 0)
                bh = feat.params.get('bbox_h', 0)
            ...
            # 從 pipe_centerlines 讀取 face_extent
            face_ext = pipe_data.get('face_extent') if pipe_data else None

            # 優先使用 (bbox_max + face_extent) / 2，排除底板影響
            if bbox_max > 0 and face_ext and face_ext > 0:
                total_len = (bbox_max + face_ext) / 2
                source = f'avg(bbox={bbox_max:.1f},fe={face_ext:.1f})'
            elif bbox_max > 0:
                total_len = bbox_max
                source = 'bbox_max'
...
====================================="""
    ...
    def _draw_title_block(self, msp, pw, ph, info_dict):
        ...
        # 外框
        msp.add_line((tb_left, tb_y), (tb_right, tb_y))
        msp.add_line((tb_left, tb_top), (tb_right, tb_top))
        msp.add_line((tb_left, tb_y), (tb_left, tb_top))
        msp.add_line((tb_right, tb_y), (tb_right, tb_top))

        # ---- 水平分隔線（4 條 = 5 列）----
        row_h = 9 * scale_factor
        row_ys = [tb_y + row_h * i for i in range(1, 5)]
        for ry in row_ys:
    ...
    def _draw_balloon(self, msp, label, balloon_pt, target_pt,
        ...
        """
        繪製業界標準球號標註（balloon annotation）。

        業界慣例：
          - 圓形氣泡（balloon）內置球號文字，文字水平置中
          - 細引出線（leader）從圓邊緣到零件表面
          - 引出線末端帶箭頭指向零件

        Args:
            msp: DXF modelspace
            label: 球號文字 (e.g. 'U1', 'D1', '1')
            balloon_pt: 氣泡中心座標 (x, y)
            target_pt: 引出線指向的零件位置 (x, y)
            radius: 氣泡圓半徑（紙面 mm）
            text_height: 文字高度
            color: DXF 顏色索引（7=白/黑, 1=紅）
        """
        import math
        bx, by = balloon_pt
        tx, ty = target_pt
        ...
        # 氣泡圓
        msp.add_circle((bx, by), radius, dxfattribs={'color': color})

        # 球號文字（置中於圓內）
        msp.add_text(str(label), dxfattribs={
            'height': text_height, 'color': color
        }).set_placement(
            (bx, by - text_height * 0.35),
        ...
        if dist < 1e-6:
            return
        ux, uy = dx / dist, dy / dist  # 方向單位向量

        # 引出線起點（圓邊緣）
        leader_start = (bx + ux * radius, by + uy * radius)
        # 引出線終點（目標點）
        leader_end = target_pt

        msp.add_line(leader_start, leader_end, dxfattribs={'color': color})

        # 箭頭（在目標端，指向目標）
        arrow_len = 2.5
        arrow_half_w = 0.8
        # 箭頭兩翼
        ax1 = tx - ux * arrow_len + uy * arrow_half_w
        ay1 = ty - uy * arrow_len - ux * arrow_half_w
        ax2 = tx - ux * arrow_len - uy * arrow_half_w
        ay2 = ty - uy * arrow_len + ux * arrow_half_w
    ...
    def _draw_dimension_line(self, msp, p1, p2, offset, text, vertical=False):
        ...
        if vertical:
            ...
            msp.add_line((dim_x, y2), (dim_x + 1, y2 - arrow_l))
            # 文字（旋轉 90 度放在尺寸線內側 — 靠近軌道的一面，標準工程圖樣式）
            if offset > 0:
                _txt_x = dim_x - 8     # 正偏移 → 文字在左側（靠近軌道）
            else:
                _txt_x = dim_x + 2     # 負偏移 → 文字在右側（靠近軌道）
            msp.add_text(text, dxfattribs={
                'height': th, 'rotation': 90
            }).set_placement((_txt_x, (y1 + y2) / 2 - 3))
        else:
            # 水平尺寸
            dim_y = p1[1] + offset
    ...
    # ================================================================
    # 共用尺寸標註函式 (Shared Dimension Annotation Helpers)
    # ================================================================

    def _annotate_track_total_length(self, msp, seg_positions, offset,
                                     total_len_override=None):
        """標註軌道總展開長（上軌或下軌）。
        seg_positions: list of (sx, sy, ex, ey, seg_len, angle_deg, label)
        offset: 尺寸線法向偏移量
        total_len_override: 若提供，使用此值取代 seg_positions 的長度加總
                           （用於包含弧段中心線長度的總展開長）
        """
        if not seg_positions:
            return
        first = seg_positions[0]
        last = seg_positions[-1]
        if total_len_override is not None:
            total_len = total_len_override
        else:
            total_len = sum(s[4] for s in seg_positions)
        self._draw_dimension_line_along(
            msp, (first[0], first[1]), (last[2], last[3]),
            offset, f"{total_len:.1f}")

    def _annotate_leg_block(self, msp, leg_top, leg_foot, leg_lower_pt,
                            ll, below_len, leg_idx, scale):
        """繪製腳架底座 + 尺寸標註 + BOM 球號（完整腳架標註區塊）。
        leg_top/leg_foot/leg_lower_pt: 腳架頂端/底端/下軌交點座標
        ll: 腳架線長, below_len: 下軌以下高度, leg_idx: 0-based 索引
        """
        # 支撐架底座
        col_h = 12 * scale
        col_w = 4
        ...
        for gi in range(int(plate_w * 2 / 2)):
            ...
            msp.add_line((gx, col_bot_y - 1.5), (gx - 2, col_bot_y - 4))

        # 腳架長度標註已移至 BOM 表備註欄（線長L=xxx），不在圖面標示

        # BOM 球號氣泡
        balloon_pt = (leg_foot[0] + 18, leg_foot[1] - 8)
        target_pt = (leg_foot[0], leg_foot[1])
        self._draw_balloon(msp, str(leg_idx + 1), balloon_pt, target_pt,
                           radius=6.0, text_height=5.0, color=7)

    def _annotate_pipe_diameter(self, msp, position, pipe_diameter, pipe_hw):
        """標註管徑 Ø（在指定位置上方）。"""
        if pipe_diameter <= 0:
            return
        msp.add_text(f"\u00D8{pipe_diameter:.1f}", dxfattribs={
            'height': 5.0
        }).set_placement((position[0], position[1] + pipe_hw + 3))

    def _annotate_rail_spacing(self, msp, upper_pt, lower_pt, offset,
                               rail_spacing):
        """標註上下軌距離（垂直尺寸線）。"""
        if rail_spacing <= 0:
            return
        self._draw_dimension_line(msp, lower_pt, upper_pt, offset,
                                  f"{rail_spacing:.1f}", vertical=True)

    def _annotate_arc_radius_leader(self, msp, leader_points, radius_val,
                                    text_height=8.0, arrow_idx=-1):
        """繪製弧半徑引線標註 R{val}。
        leader_points: list of (x,y) — 引線折點
        arrow_idx: 箭頭所在折點索引 (-1=最末, None=不畫箭頭)
        """
        # 引線段
        for i in range(len(leader_points) - 1):
            msp.add_line(leader_points[i], leader_points[i + 1])
        # 箭頭
        if arrow_idx is not None and len(leader_points) >= 2:
            idx = arrow_idx if arrow_idx >= 0 else len(leader_points) + arrow_idx
            if 0 < idx < len(leader_points):
                pt = leader_points[idx]
                prev = leader_points[idx - 1]
                dx = pt[0] - prev[0]
                dy = pt[1] - prev[1]
                ...
                if d > 1e-6:
                    ux, uy = dx / d, dy / d
                    al = 3.0
                    msp.add_line(pt, (pt[0] - ux*al + uy*al*0.3,
                                      pt[1] - uy*al - ux*al*0.3))
                    msp.add_line(pt, (pt[0] - ux*al - uy*al*0.3,
                                      pt[1] - uy*al + ux*al*0.3))
        # R 文字
        text_pt = leader_points[-1]
        msp.add_text(f"R{radius_val:.0f}", dxfattribs={
            'height': text_height
        }).set_placement((text_pt[0] + 2, text_pt[1] + 1))

    def _annotate_elevation_angle(self, msp, position, angle_deg,
                                  x_dir=1, ref_len=20):
        """標註基礎仰角（垂直參考線 + 從垂直線量測的角度文字）。"""
        complement = 90 - angle_deg
        ref_x = position[0]
        msp.add_line((ref_x, position[1] + ref_len),
                     (ref_x, position[1] - ref_len),
                     dxfattribs={'color': 8})
    ...
    def _draw_bend_arc(self, msp, center, prev_angle, post_angle,
        ...
        Args:
            outside: True = 弧線畫在外側（+180°）
        """
        cx, cy = center
        arc_r = min(15, 25 * scale)
        if x_dir < 0:
            pre_dxf = 180 - prev_angle
            post_dxf = 180 - post_angle
        else:
            pre_dxf = prev_angle
            post_dxf = post_angle
        ...
        txt_r = arc_r + (12 if outside else 5)
        msp.add_text(f"{bend_deg:.0f}\u00B0", dxfattribs={
            'height': 5.0
        }).set_placement((cx + txt_r * math.cos(mid_rad),
                          cy + txt_r * math.sin(mid_rad)))
    ...
    def _draw_top_plan_view(self, msp, area_x, area_y, area_w, area_h,
        ...
        R_arc = arc_radius
        bracket_arm_ext = pipe_diameter * 1.5  # 支架臂延伸到管壁外的距離
        ...
        # Bounding box（包含軌道外壁 + 支架向凹側延伸）
        bb_out_margin = pipe_r + 15  # 外側：軌道管壁 + 餘裕
        bb_in_margin = pipe_r + bracket_arm_ext + 15  # 內側：含支架延伸
        bb_pts = []
        for i in range(101):
            t = i / 100
            ...
            # 外側弧
            bb_pts.append((arc_cx + (R_arc + bb_out_margin) * ca,
                           arc_cy + (R_arc + bb_out_margin) * sa))
            # 內側弧（支架端部）
            bb_pts.append((arc_cx + max(0, R_arc - bb_in_margin) * ca,
                           arc_cy + max(0, R_arc - bb_in_margin) * sa))
        ...
        _draw_arc_poly(arc_cx, arc_cy, R_arc,
                       plan_sa_deg, plan_span_deg, color=1, linetype='CENTER')
        ...
        # 支撐架標記 — 以圓柱延長顯示（俯視：雙管壁線 + 端部圓柱截面）
        # 標準表示法（參考 2-2-2.jpg）:
        #   從軌道中心線向弧心方向（凹側/下側）延伸，
        #   繪製雙管壁平行線（實際管徑 d≈13mm）+
        #   端部小圓（圓柱端面，實際管徑）
        if bracket_count > 0:
            # 取支撐架實際管徑（從 pipe_centerlines 讀取）
            _brk_pipes = [pc for pc in stp_data.get('pipe_centerlines', [])
                          if ({c['feature_id']: c for c in stp_data.get(
                              'part_classifications', [])}).get(
                              pc['solid_id'], {}).get('class') == 'bracket']
            _brk_pipe_diam = 0
            if _brk_pipes:
                _brk_pipe_diam = _brk_pipes[0].get('diameter', 0)
            if _brk_pipe_diam <= 0:
                _brk_pipe_diam = 13  # 典型支撐架管徑
            bracket_circle_r = _brk_pipe_diam / 2 * view_scale  # 紙面半徑
            bracket_circle_r = max(bracket_circle_r, 1.0)   # 最小可見
            _brk_hw = _brk_pipe_diam / 2 * view_scale  # 管壁半寬（紙面）
            _brk_hw = max(_brk_hw, 0.5)

            # 支撐架延伸長度（俯視投影）
            _brk_arm_model = 0   # 水平臂長
            _brk_vert_model = 0  # 垂直段長
            if _brk_pipes:
                for seg in _brk_pipes[0].get('segments', []):
                    if seg.get('type') == 'straight':
                        dx, dy, dz = seg.get('direction', (0, 0, 0))
                        horiz = math.sqrt(dx**2 + dy**2)
                        vert = abs(dz)
                        if horiz > vert:
                            _brk_arm_model = max(_brk_arm_model, seg['length'])
                        else:
                            _brk_vert_model = max(_brk_vert_model, seg['length'])

            # 仰角造成的垂直段水平投影
            elevation_deg = stp_data.get('elevation_deg', 0)
            _vert_proj = (_brk_vert_model * math.sin(math.radians(elevation_deg))
                          if elevation_deg > 0 else 0)
            _total_ext_model = _brk_arm_model + _vert_proj
            if _total_ext_model < 10:
                _total_ext_model = pipe_diameter * 1.0  # fallback
            # 俯視圖中支撐架僅為短柱狀示意，限制上限以符合標準圖比例
            _total_ext_model = min(_total_ext_model, pipe_diameter * 1.0)
            ...
            for bi in range(bracket_count):
                ...
                # 根部（軌道中心線上）
                root_mx = arc_cx + R_arc * cos_ba
                root_my = arc_cy + R_arc * sin_ba
                root_pt = _m2d(root_mx, root_my)

                # 外端（從軌道中心線向弧心方向延伸，即下側/凹側）
                # 標準圖中支撐架懸掛於軌道下方，俯視投影在弧心側（凹側）
                tip_mx = root_mx - _total_ext_model * cos_ba
                tip_my = root_my - _total_ext_model * sin_ba
                tip_pt = _m2d(tip_mx, tip_my)

                # 徑向單位向量（紙面座標）及法線
                ldx = tip_pt[0] - root_pt[0]
                ldy = tip_pt[1] - root_pt[1]
                ll = math.sqrt(ldx**2 + ldy**2)
                if ll > 1e-9:
                    nx_p, ny_p = -ldy / ll * _brk_hw, ldx / ll * _brk_hw
                else:
                    nx_p, ny_p = 0, _brk_hw

                # 雙管壁平行線（管徑 = 支撐架直徑）
                msp.add_line((root_pt[0] + nx_p, root_pt[1] + ny_p),
                             (tip_pt[0] + nx_p, tip_pt[1] + ny_p))
                msp.add_line((root_pt[0] - nx_p, root_pt[1] - ny_p),
                             (tip_pt[0] - nx_p, tip_pt[1] - ny_p))
                # 外端封口
                msp.add_line((tip_pt[0] + nx_p, tip_pt[1] + ny_p),
                             (tip_pt[0] - nx_p, tip_pt[1] - ny_p))
                # 外端圓柱截面（實際管徑）
                msp.add_circle(tip_pt, bracket_circle_r)

        # R 半徑標註 — flag: show_basic_info
        _SHOW_BASIC_TPV = self._drafter_config.get('show_basic_info', False)
        if _SHOW_BASIC_TPV:
            r_label_angle = math.radians(plan_sa_deg + plan_span_deg * 0.8)
            center_pt = _m2d(arc_cx, arc_cy)
            r_arc_pt = _m2d(arc_cx + R_arc * math.cos(r_label_angle),
                            arc_cy + R_arc * math.sin(r_label_angle))
            r_dir_x = r_arc_pt[0] - center_pt[0]
            r_dir_y = r_arc_pt[1] - center_pt[1]
            r_dir_len = math.sqrt(r_dir_x**2 + r_dir_y**2)
            if r_dir_len > 1e-9:
                r_dir_x, r_dir_y = r_dir_x / r_dir_len, r_dir_y / r_dir_len
            else:
                r_dir_x, r_dir_y = 0, 1
            r_text_end = (r_arc_pt[0] + r_dir_x * 15,
                          r_arc_pt[1] + r_dir_y * 15)
            self._annotate_arc_radius_leader(
                msp, [center_pt, r_arc_pt, r_text_end],
                arc_radius, text_height=7.0, arrow_idx=1)

        # 計算軌道外壁的總寬度（不含支架延伸）
        _all_mx = []
        for _ea_i in [plan_sa_deg, plan_sa_deg + plan_span_deg]:
            _ea_r = math.radians(_ea_i)
            _all_mx.append(arc_cx + (R_arc + pipe_r) * math.cos(_ea_r))
            _all_mx.append(arc_cx + (R_arc - pipe_r) * math.cos(_ea_r))
        # 也包含弧線極值點
        for i in range(101):
            t = i / 100
            a = math.radians(plan_sa_deg + t * plan_span_deg)
            _all_mx.append(arc_cx + (R_arc + pipe_r) * math.cos(a))
            _all_mx.append(arc_cx + (R_arc - pipe_r) * math.cos(a))
        bracket_total_width = max(_all_mx) - min(_all_mx) if _all_mx else 0
        ...
        return {
            ...
            'plan_valid': True,
            'R_arc': R_arc,
            'bracket_arm_ext': bracket_arm_ext,
            'bracket_total_width': bracket_total_width,
            'pipe_r': pipe_r,
        }
    ...
    def _hlr_project_to_polylines(self, main_dir_xyz, x_dir_xyz, flip_v=False,
        ...
        Args:
            ...
            flip_v: 是否翻轉垂直方向
            solid_ids: 若指定，僅投影這些 feature_id 的 solid（用於過濾）
        ...
        # 若指定 solid_ids，組合指定的 solid shapes；否則用整個模型
    ...
    def generate_overview_drawing(self, output_dir, base_name, stp_data, project, today):
        ...
        if iso_polylines:
            ...
            # 繪製斜向尺寸線（上軌兩端點內側長度）— flag: show_basic_info
            _SHOW_BASIC_D0 = self._drafter_config.get('show_basic_info', False)
            if _SHOW_BASIC_D0:
                ...
                # 尺寸線放在軌道外側：計算模型重心方向，offset 往反方向
                all_iso_pts = [project_iso(ep) for ep in all_endpoints]
                center_iso = (sum(p[0] for p in all_iso_pts) / len(all_iso_pts),
                              sum(p[1] for p in all_iso_pts) / len(all_iso_pts))
                center_paper = (center_iso[0] * iso_scale + iso_off_x,
                                center_iso[1] * iso_scale + iso_off_y)
                mid_dim = ((dim_a[0] + dim_b[0]) / 2, (dim_a[1] + dim_b[1]) / 2)
                dx_dim = dim_b[0] - dim_a[0]
                dy_dim = dim_b[1] - dim_a[1]
                seg_l = math.sqrt(dx_dim**2 + dy_dim**2)
                if seg_l > 1e-6:
                    nx_dim = -dy_dim / seg_l   # 左側法線
                    ny_dim = dx_dim / seg_l
                    dot_c = (center_paper[0] - mid_dim[0]) * nx_dim + \
                            (center_paper[1] - mid_dim[1]) * ny_dim
                    dim_offset = -25 if dot_c > 0 else 25
                else:
                    dim_offset = 25
                self._draw_dimension_line_along(msp, dim_a, dim_b, dim_offset, dim_text)

        # ==== 右半：上視圖（正投影，X 軸旋轉 180°，僅含軌道管路） ====
        # 從正上方（+Z）往下看，model_Y 向右
        # flip_v=True → screen_Y = -model_X（X 軸旋轉 180°，follow 標準圖方向）
        # 只投影 track 管路，排除腳架/支撐架等，保持圖面清潔
        top_main = (0, 0, -1)  # 從上往下看（視線方向 -Z）
        top_x = (0, 1, 0)     # model Y 向右
        track_solid_ids = [tp['solid_id'] for tp in track_pipes]
        top_polylines, top_bbox = self._hlr_project_to_polylines(
            top_main, top_x, flip_v=True, solid_ids=track_solid_ids)
        ...
        if top_polylines:
            ...
            # 尺寸線端點：screen_y = -model_X，外壁加管壁半徑
            pipe_r_screen = pipe_diameter_top / 2
            dim_left_x = bx0 * top_scale + top_off_x - 12
            # X 軸旋轉 180° 後：model_X=x_max 對應 screen_y 較低
            ...
            # 弧半徑標註：R250 — flag: show_basic_info
            arc_pipe = None
            for tp in track_pipes:
                for seg in tp.get('segments', []):
                    if seg.get('type') == 'arc' and seg.get('radius', 0) > 50:
            ...
            if _SHOW_BASIC_D0 and arc_pipe:
                sp_3d = arc_pipe.get('start_point', (0, 0, 0))
                ep_3d = arc_pipe.get('end_point', (0, 0, 0))

                # 弧的凸側（外側）：上視圖中 screen_x = model_Y
                # 弧的外側為 max(Y) 方向（螢幕右側）
                y_max_arc = max(sp_3d[1], ep_3d[1])
                arc_mid_x = (sp_3d[0] + ep_3d[0]) / 2
                pipe_r = stp_data['pipe_diameter'] / 2
                # 引線起點：弧外壁 (model_Y 最大 + 管壁半徑方向)
                arc_outer_3d = (arc_mid_x, y_max_arc + pipe_r, 0)
                r_proj = _project_top(arc_outer_3d)
                r_start = (r_proj[0] * top_scale + top_off_x,
                           r_proj[1] * top_scale + top_off_y)

                # 引線向右上方延伸（共用弧半徑引線標註）
                r_ext = 30
                r_tip = (r_start[0] + r_ext, r_start[1] + r_ext * 0.6)
                self._annotate_arc_radius_leader(
                    msp, [r_start, r_tip],
                    arc_radius, text_height=8.0, arrow_idx=None)
    ...
    def _draw_straight_section_sheet(self, section, section_bends,
        ...
        # === 標示開關（從 config 讀取，default 關）===
        _SHOW_BASIC = self._drafter_config.get('show_basic_info', False)
        _SHOW_LEG_ANGLES = self._drafter_config.get('show_leg_angles', False)
        ...
        # 計算本段的實際軌距（perpendicular distance between upper/lower centerlines）
        # 優先使用 3D 管中心線資料，避免使用全域平均值
        section_rail_spacing = rail_spacing  # fallback
        upper_tracks = section.get('upper_tracks', [])
        lower_tracks = section.get('lower_tracks', [])
        _pcl_map_sp = {}
        if stp_data and 'pipe_centerlines' in stp_data:
            for _pc in stp_data['pipe_centerlines']:
                _pcl_map_sp[_pc['solid_id']] = _pc
        if upper_tracks and lower_tracks and _pcl_map_sp:
            _ut_id = upper_tracks[0].get('solid_id', '')
            _lt_id = lower_tracks[0].get('solid_id', '')
            _ut_pc = _pcl_map_sp.get(_ut_id, {})
            _lt_pc = _pcl_map_sp.get(_lt_id, {})
            _ut_sp = _ut_pc.get('start_point')
            _ut_ep = _ut_pc.get('end_point')
            _lt_sp = _lt_pc.get('start_point')
            if _ut_sp and _ut_ep and _lt_sp:
                _d3 = tuple(_ut_ep[i] - _ut_sp[i] for i in range(3))
                _mag3 = math.sqrt(sum(d**2 for d in _d3))
                if _mag3 > 1:
                    _udir = tuple(d / _mag3 for d in _d3)
                    _ab = tuple(_lt_sp[i] - _ut_sp[i] for i in range(3))
                    _cross = (_udir[1]*_ab[2] - _udir[2]*_ab[1],
                              _udir[2]*_ab[0] - _udir[0]*_ab[2],
                              _udir[0]*_ab[1] - _udir[1]*_ab[0])
                    _perp_dist = math.sqrt(sum(c**2 for c in _cross))
                    if _perp_dist > 10:
                        section_rail_spacing = _perp_dist
                        logger.info(f"  Section rail_spacing={section_rail_spacing:.1f}"
                                    f" (vs global={rail_spacing:.1f})")
        ...
        # upper_tracks / lower_tracks 已於 section_rail_spacing 計算時讀取
        ...
        def _ensure_main_first(cl_items):
            """若主管段在最後面，反轉 cutting list 使主管段在最前
            Returns: (cl_items, reversed_flag)"""
            first_str = next((it for it in cl_items
                              if it.get('type') == 'straight'), None)
            if first_str and first_str.get('solid_id'):
        ...
        if _start_is_transition or _is_drawing1:
            ...
            upper_cl, _upper_cl_reversed = _ensure_main_first(upper_cl)
            lower_cl, _lower_cl_reversed = _ensure_main_first(lower_cl)
        ...
        # 上軌/下軌起始角修正：
        # 判斷主管段（有 solid_id）在前還是在後，決定起始角計算方式：
        # - 主管段在最後（過渡段→彎→主管）：start = base - total_bend
        # - 主管段在最前（主管→彎→過渡段）：start = base（不調整）
        # bend_sign: +1=上彎(仰角遞增), -1=下彎(仰角遞減)
        def _calc_start_elev(cl_items, base_elev):
            bend_total = sum(
                it.get('bend_sign', 1) * it.get('angle_deg', 0)
                for it in cl_items if it.get('type') == 'arc')
            # 檢查第一個直線段是否為主管段（有 solid_id）
            first_straight = next(
                (it for it in cl_items if it.get('type') == 'straight'), None)
            if first_straight and first_straight.get('solid_id'):
                # 主管段有 solid_id → 但需檢查前面是否有弧段
                # （反轉後的下軌可能是 [弧段, 主管段]）
                arcs_before = 0
                for it in cl_items:
                    if it.get('type') == 'straight':
                        break  # 到達第一個直線段
                    if it.get('type') == 'arc':
                        arcs_before += (it.get('bend_sign', 1)
                                        * it.get('angle_deg', 0))
                if abs(arcs_before) > 0.01:
                    # 弧段在主管段前面 → 起始角 = base - arcs_before
                    return base_elev - arcs_before
                return base_elev
            else:
                # 過渡段在前，彎曲在前 → 起始角 = base_elev - total_bend
                return base_elev - bend_total

        upper_start_elev = _calc_start_elev(upper_cl, upper_base_elev)
        ...
        _main_a_u = next(
            ...
            upper_path[0]['angle_deg'] if upper_path else 0)
        _main_a_l = next(
        ...
        def _calc_path_y_displacement(path_info):
            ...
            for it in path_info:
                if it['type'] == 'straight':
                    y += it['length'] * math.sin(math.radians(it['angle_deg']))
                elif it['type'] == 'arc':
                    R = it['radius']
                    prev_a = math.radians(it['prev_angle'])
                    bs = it.get('bend_sign', 1)
                    end_a = prev_a + bs * math.radians(it['angle_deg'])
                    y += bs * R * (math.cos(prev_a) - math.cos(end_a))
            return y
        ...
        def _calc_path_x_displacement(path_info):
            ...
            for it in path_info:
                if it['type'] == 'straight':
                    x += it['length'] * math.cos(math.radians(it['angle_deg']))
                elif it['type'] == 'arc':
                    R = it['radius']
                    prev_a = math.radians(it['prev_angle'])
                    bs = it.get('bend_sign', 1)
                    end_a = prev_a + bs * math.radians(it['angle_deg'])
                    x += bs * R * (math.sin(end_a) - math.sin(prev_a))
            return x
        _upper_x_disp = _calc_path_x_displacement(upper_path)
        _lower_x_disp = _calc_path_x_displacement(lower_path)

        # 各腳架垂直高度（下軌以下的長度）
        leg_below_heights = []
        for li, leg in enumerate(section_legs):
            ll = leg.get('line_length', 0)  # from stp_data (cutting_list)
            through = main_vert_gap
        ...
        def _compute_path_extent(path_info, start_x, start_y):
            ...
            for item in path_info:
                if item['type'] == 'straight':
                    ...
                    # 弧段位移（弧線在空間中佔有實際距離）
                    R = item['radius']
                    prev_a = math.radians(item['prev_angle'])
                    bs = item.get('bend_sign', 1)
                    end_a = prev_a + bs * math.radians(item['angle_deg'])
                    cx += bs * R * (math.sin(end_a) - math.sin(prev_a))
                    cy += bs * R * (math.cos(prev_a) - math.cos(end_a))
                    all_x.append(cx)
                    all_y.append(cy)
            return all_x, all_y
        ...
        # 下軌偏移 -main_vert_gap（垂直間距）
        lo_ys_offset = [y - main_vert_gap for y in lo_ys]
        ...
        def _draw_cl_path(msp, path_info, start_x, start_y, scale, pipe_hw, dim_side):
            ...
            回傳 (seg_positions, bend_infos):
              seg_positions: list of (sx, sy, ex, ey, length, angle_deg, label)
              bend_infos: list of dict with bend center/angles info
            ...
            for item in path_info:
                if item['type'] == 'straight':
                    ...
                    # 段長標註（沿軌道方向）— flag: show_basic_info
                    if _SHOW_BASIC and draw_len > 5:
                        self._draw_dimension_line_along(
                            msp, (sx, sy), (ex, ey),
                            dim_side * x_dir * (pipe_hw + 8),
                            f"{seg_len:.1f}")

                    # 取料球號標註（共用 _draw_cl_label）— flag: show_basic_info
                    if _SHOW_BASIC and label and draw_len > 5:
                    ...
                    post_rad = math.radians(post_angle)
                    arc_dx = (x_dir * bend_sign * bend_r
                              * (math.sin(post_rad) - math.sin(prev_rad))
                              * scale)
                    arc_dy = (bend_sign * bend_r
                              * (math.cos(prev_rad) - math.cos(post_rad))
                              * scale)

                    # 繪製弧段管壁（折線近似）
                    arc_sx, arc_sy = cx, cy
                    n_arc_seg = max(8, int(bend_deg))
                    prev_pt = (cx, cy)
                    for ai in range(1, n_arc_seg + 1):
                        frac = ai / n_arc_seg
                        a_cur = prev_angle + bend_sign * bend_deg * frac
                        a_cur_rad = math.radians(a_cur)
                        # 弧上的點
                        apx = (arc_sx + x_dir * bend_sign * bend_r
                               * (math.sin(a_cur_rad) - math.sin(prev_rad))
                               * scale)
                        apy = (arc_sy + bend_sign * bend_r
                               * (math.cos(prev_rad) - math.cos(a_cur_rad))
                               * scale)
                        # 前一點和當前點的法線（管壁偏移）
                        a_prev_seg = prev_angle + bend_sign * bend_deg * ((ai - 1) / n_arc_seg)
                        a_prev_r = math.radians(a_prev_seg)
                        nxp = -math.sin(a_prev_r) * pipe_hw
                        nyp = x_dir * math.cos(a_prev_r) * pipe_hw
                        nxc = -math.sin(a_cur_rad) * pipe_hw
                        nyc = x_dir * math.cos(a_cur_rad) * pipe_hw
                        # 管壁雙線
                        msp.add_line((prev_pt[0] + nxp, prev_pt[1] + nyp),
                                     (apx + nxc, apy + nyc))
                        msp.add_line((prev_pt[0] - nxp, prev_pt[1] - nyp),
                                     (apx - nxc, apy - nyc))
                        # 中心線（紅色）
                        msp.add_line(prev_pt, (apx, apy),
                                     dxfattribs={'color': 1})
                        prev_pt = (apx, apy)
                    ...
                    # 弧段中點（用於標註和球號）
                    mid_a = prev_angle + bend_sign * bend_deg * 0.5
                    mid_a_rad = math.radians(mid_a)
                    mid_x = (arc_sx + x_dir * bend_sign * bend_r
                             * (math.sin(mid_a_rad) - math.sin(prev_rad))
                             * scale)
                    mid_y = (arc_sy + bend_sign * bend_r
                             * (math.cos(prev_rad) - math.cos(mid_a_rad))
                             * scale)
                    ...
                    bend_infos.append(bend_info)

                    # 彎曲段取料球號標註 — flag: show_basic_info
                    arc_label = item.get('label', '')
                    if _SHOW_BASIC and arc_label:
                        self._draw_cl_label(msp, arc_label, (mid_x, mid_y),
                                            dim_side)

            # 端蓋（首段起點 + 軌道終點）
            _last_angle = 0
            for item in reversed(path_info):
                if item['type'] == 'straight':
                    _last_angle = item['angle_deg']
                    break
                elif item['type'] == 'arc':
                    _last_angle = item['prev_angle'] + item.get('bend_sign', 1) * item['angle_deg']
                    break
            ...
            if seg_positions:
                ...
                if seg_d > 1e-6:
                    ...
                    nx, ny = 0, pipe_hw
                msp.add_line((sp0[0] + nx, sp0[1] + ny),
                             (sp0[0] - nx, sp0[1] - ny))

                # 軌道終點端蓋（可能在最後直線段末端或弧段末端）
                end_rad = math.radians(_last_angle)
                dx_e = x_dir * math.cos(end_rad)
                dy_e = math.sin(end_rad)
                d_e = math.sqrt(dx_e**2 + dy_e**2)
                if d_e > 1e-6:
                    nx = -dy_e / d_e * pipe_hw
                    ny = dx_e / d_e * pipe_hw
                else:
                    nx, ny = 0, pipe_hw
                msp.add_line((cx + nx, cy + ny), (cx - nx, cy - ny))

            return seg_positions, bend_infos, (cx, cy)

        # 繪製上軌完整路徑
        upper_seg_positions, upper_bends, upper_track_end = _draw_cl_path(
            msp, upper_path, base_x, base_y_upper, scale, pipe_hw, +1)

        # 下軌起始點：上軌起始點正下方 main_vert_gap（垂直間距）
        lower_start_y = base_y_upper - main_vert_gap * scale
        lower_seg_positions, lower_bends, lower_track_end = _draw_cl_path(
            msp, lower_path, base_x, lower_start_y, scale, pipe_hw, -1)

        # (管徑標註移至下方 _SHOW_ANNOTATIONS 區塊，避免重複)
        # (軌道間距標註已由 Circle 2「上下軌中心線垂直距離」取代)

        # ========== 整體尺寸標註 — 總長度 always on ==========
        self._annotate_total_lengths(
            msp, upper_path, lower_path,
            upper_seg_positions, lower_seg_positions,
        ...
        # 水平投影方向（忽略 Z 分量，避免腳架垂直偏移影響投影）
        _horiz_dir = (track_dir_3d[0], track_dir_3d[1], 0)
        _horiz_mag = math.sqrt(_horiz_dir[0]**2 + _horiz_dir[1]**2)
        if _horiz_mag > 1e-6:
            _horiz_dir = (_horiz_dir[0] / _horiz_mag,
                          _horiz_dir[1] / _horiz_mag, 0)
        else:
            _horiz_dir = (0, 1, 0)

        def _proj_horiz(pt):
            """將 3D 點投影到軌道水平方向（忽略垂直偏移）"""
            return pt[0] * _horiz_dir[0] + pt[1] * _horiz_dir[1]

        # 軌道水平投影範圍（含上下軌，用於腳架定位）
        all_track_h = []
        for ut in upper_tracks:
            _fid = ut.get('solid_id', '')
            pd = _pcl_map_draw.get(_fid, ut.get('pipe_data', {}))
            sp3 = pd.get('start_point', (0, 0, 0))
            ep3 = pd.get('end_point', (0, 0, 0))
            all_track_h.extend([_proj_horiz(sp3), _proj_horiz(ep3)])
        for lt in lower_tracks:
            _fid = lt.get('solid_id', '')
            pd = _pcl_map_draw.get(_fid, lt.get('pipe_data', {}))
            sp3 = pd.get('start_point', (0, 0, 0))
            ep3 = pd.get('end_point', (0, 0, 0))
            all_track_h.extend([_proj_horiz(sp3), _proj_horiz(ep3)])
        if all_track_h:
            track_h_min = min(all_track_h)
            track_h_max = max(all_track_h)
            track_h_span = max(track_h_max - track_h_min, 1)
        else:
            track_h_min, track_h_max, track_h_span = 0, 1, 1

        # 判斷水平投影方向是否與繪圖路徑方向相反
        # 策略：找 CL 中有 solid_id 的 straight 段，判斷它在路徑的前段還是後段，
        #       再比較其 3D 水平投影位置與整體中點，決定是否反轉 t 映射。
        # _h_inverted=True 表示路徑起點在 h_max 側（h 沿路徑遞減）
        _cl_straights_with_id = [
            (i, it) for i, it in enumerate(upper_cl)
            if it.get('type') == 'straight' and it.get('solid_id')]
        if _cl_straights_with_id and all_track_h:
            # 優先用第一個有 solid_id 的 straight 段
            _idx_in_cl, _cl_ref = _cl_straights_with_id[0]
            _is_near_start = (_idx_in_cl <= len(upper_cl) // 2)
            _fid_fc = _cl_ref['solid_id']
        ...
        if upper_seg_positions:
            ...
            path_end = upper_track_end  # 使用實際軌道終點（含弧段位移）
        else:
        ...
        leg_draw_data = []  # 收集每個腳架的繪圖位置資訊（用於後續標註）
        ...
        for li, leg in enumerate(section_legs):
            ...
            # === 腳架在繪圖路徑上的位置 ===
            leg_centroid = leg.get('centroid', (0, 0, 0))
            # 策略：CL 第一個 straight 有 solid_id → 用水平投影法（精度高）
            #       CL 第一個 straight 無 solid_id（過渡段）→ 用 3D 近似投影法
            _first_str = next(
                (it for it in upper_cl if it.get('type') == 'straight'), None)
            _use_h_proj = bool(
                _first_str and _first_str.get('solid_id') and all_track_h)

            if _use_h_proj:
                # --- 水平投影法 ---
                leg_h = _proj_horiz(leg_centroid)
                if _h_inverted:
                    t = (track_h_max - leg_h) / track_h_span
                else:
                    t = (leg_h - track_h_min) / track_h_span
                t = max(0.05, min(0.95, t))
                logger.info(
                    f"  Leg[{li}](h-proj): centroid=({leg_centroid[0]:.0f},"
                    f"{leg_centroid[1]:.0f},{leg_centroid[2]:.0f}), "
                    f"leg_h={leg_h:.1f}, t={t:.3f}, h_inv={_h_inverted}, "
                    f"h_range=[{track_h_min:.1f},{track_h_max:.1f}]")
            else:
                # --- 3D 近似投影法：投影到最近的 CL straight(有 solid_id) ---
                _total_model_len = 0
                for _ci in upper_cl:
                    if _ci.get('type') == 'straight':
                        _total_model_len += _ci.get('length', 0)
                    elif _ci.get('type') == 'arc':
                        _total_model_len += _ci.get('outer_arc_length', 0)
                _best_t_leg = 0.5
                _best_dist_leg = float('inf')
                _cl_cum = 0
                for _ci in upper_cl:
                    _ci_len = (_ci.get('length', 0) if _ci.get('type') == 'straight'
                               else _ci.get('outer_arc_length', 0))
                    if _ci.get('type') == 'straight' and _ci.get('solid_id'):
                        _fid_leg = _ci['solid_id']
                        _pd_leg = _pcl_map_draw.get(_fid_leg, {})
                        _sp_leg = _pd_leg.get('start_point', (0, 0, 0))
                        _ep_leg = _pd_leg.get('end_point', (0, 0, 0))
                        _sd = (_ep_leg[0] - _sp_leg[0],
                               _ep_leg[1] - _sp_leg[1],
                               _ep_leg[2] - _sp_leg[2])
                        _sl = math.sqrt(sum(d**2 for d in _sd))
                        if _sl > 1:
                            _su = tuple(d / _sl for d in _sd)
                            _to_leg = (leg_centroid[0] - _sp_leg[0],
                                       leg_centroid[1] - _sp_leg[1],
                                       leg_centroid[2] - _sp_leg[2])
                            _proj_d = sum(a * b for a, b in zip(_to_leg, _su))
                            _frac = max(0, min(1, _proj_d / _sl))
                            _cp = tuple(
                                _sp_leg[i] + _frac * _sd[i] for i in range(3))
                            _dist = math.sqrt(sum(
                                (leg_centroid[i] - _cp[i])**2 for i in range(3)))
                            if _dist < _best_dist_leg:
                                _best_dist_leg = _dist
                                _leg_cum = _cl_cum + _frac * _ci_len
                                _best_t_leg = (_leg_cum / _total_model_len
                                               if _total_model_len > 0 else 0.5)
                    _cl_cum += _ci_len
                t = max(0.05, min(0.95, _best_t_leg))
                logger.info(
                    f"  Leg[{li}](3d-prox): centroid=({leg_centroid[0]:.0f},"
                    f"{leg_centroid[1]:.0f},{leg_centroid[2]:.0f}), "
                    f"t={t:.3f}, best_dist={_best_dist_leg:.1f}, "
                    f"total_model_len={_total_model_len:.1f}")
            leg_draw_x, upper_y = _interp_path(t)

            # 下軌 Y：使用下軌實際路徑在 leg_draw_x 處的 Y 值
            # （非固定偏移，避免上下軌角度差造成的位置偏差）
            lower_y = _find_y_on_path(lower_seg_positions, leg_draw_x,
                                      upper_y - main_vert_gap * scale)

            leg_upper_pt = (leg_draw_x, upper_y)
            leg_lower_pt = (leg_draw_x, lower_y)

            # 腳架幾何
            # 使用上下軌在腳架位置的實際視覺間距（非固定 rail_spacing），
            # 確保腳架繪製長度 = ll * scale（比例正確）。
            visual_through = max(1, (upper_y - lower_y) / scale)
            remain = max(0, ll - visual_through)
            # 上方延伸約 26%（基於標準圖分析：leg L=510 → above≈74）
            above_len = remain * 0.26
            ...
            # 收集腳架繪圖數據（用於後續標註）
            leg_draw_data.append({
                'draw_x': leg_draw_x, 'upper_y': upper_y,
                'lower_y': lower_y, 'visual_through': visual_through,
                'leg_top': leg_top, 'leg_foot': leg_foot,
                'leg_upper_pt': leg_upper_pt, 'leg_lower_pt': leg_lower_pt,
                't': t,
            })
            ...
            # 上下軌穿越處的交叉標記 (X)
            x_sz = min(4, main_vert_gap * scale * 0.2)
            ...
            # 底座 + 尺寸標註 + BOM 球號 — flag: show_basic_info
            if _SHOW_BASIC:
        ...
        if _SHOW_LEG_ANGLES and leg_draw_data:
            self._annotate_leg_angles(
                msp, leg_draw_data,
                upper_seg_positions, lower_seg_positions,
                x_dir, scale,
                _annotation_like_draw3, upper_start_elev)

        # ========== 新增：軌距/角度/位置標註 ==========
        if _SHOW_ANNOTATIONS and upper_seg_positions and lower_seg_positions:
            # Pre-compute shared reference values
            u_sy = upper_seg_positions[0][1]
            l_sy = lower_start_y
            start_ref_x = upper_seg_positions[0][0]
            _start_gap_val = start_gap
            _end_gap_val = end_gap
            ...
            u_ex, u_ey = upper_track_end
            l_ex, l_ey = lower_track_end
            ref_end_x = max(u_ex, l_ex) if x_dir == -1 else min(u_ex, l_ex)
            end_uy = _find_y_on_path(upper_seg_positions, ref_end_x, u_ey)
            end_ly = _find_y_on_path(lower_seg_positions, ref_end_x, l_ey)

            self._annotate_vert_gap(
                msp,
                start_ref_x, u_sy, l_sy,
                ref_end_x, end_uy, end_ly,
                _start_gap_val, _end_gap_val,
                x_dir)
            self._annotate_inner_dist(
                msp,
                start_ref_x, u_sy, l_sy,
                ref_end_x, end_uy, end_ly,
                _start_gap_val, _end_gap_val,
                _angle_at_start, _angle_at_end,
                pipe_diameter, pipe_hw, x_dir)
            self._annotate_end_dist(
                msp,
                start_ref_x, u_sy, l_sy,
                ref_end_x, end_uy, end_ly,
                _start_gap_val, _end_gap_val,
                ...
                upper_seg_positions, lower_seg_positions,
                pipe_hw, x_dir)
            self._annotate_leg_to_bend(
                msp, leg_draw_data,
                upper_seg_positions,
                pipe_hw, x_dir,
                _is_drawing1, _start_is_transition, _annotation_like_draw3)

        # [MARKED] 管徑 Ø 標註 — 暫時停用，後續再評估
        # if pipe_diameter > 0 and upper_seg_positions:
        #     self._annotate_pipe_diameter(msp, ..., pipe_diameter, pipe_hw)
    ...
    def _annotate_total_lengths(
            self, msp, upper_path, lower_path,
            upper_seg_positions, lower_seg_positions,
            upper_track_end, lower_track_end,
            base_x, lower_start_y,
            pipe_hw, x_dir, _SHOW_BASIC, _start_is_transition):
        ...
        def _calc_endpoint_distance(path_info):
            """計算路徑兩端點的直線距離。"""
            x, y = 0.0, 0.0
            for it in path_info:
                if it['type'] == 'straight':
                    a = math.radians(it['angle_deg'])
                    x += it['length'] * math.cos(a)
                    y += it['length'] * math.sin(a)
                elif it['type'] == 'arc':
                    bend_deg = it['angle_deg']
                    R = it['radius']
                    prev_a = math.radians(it['prev_angle'])
                    bs = it.get('bend_sign', 1)
                    end_a = prev_a + bs * math.radians(bend_deg)
                    x += bs * R * (math.sin(end_a) - math.sin(prev_a))
                    y += bs * R * (math.cos(prev_a) - math.cos(end_a))
            return math.sqrt(x * x + y * y)

        upper_total_len = _calc_endpoint_distance(upper_path)
        lower_total_len = _calc_endpoint_distance(lower_path)

        if _SHOW_BASIC and upper_seg_positions:
            self._draw_dimension_line_along(
                msp,
                (upper_seg_positions[0][0], upper_seg_positions[0][1]),
                upper_track_end,
                x_dir * (pipe_hw + 25), f"{upper_total_len:.1f}")
        if _SHOW_BASIC and lower_seg_positions:
            if _start_is_transition:
                _lower_dim_start = (base_x, lower_start_y)
            else:
                _lower_dim_start = (lower_seg_positions[0][0],
                                    lower_seg_positions[0][1])
            self._draw_dimension_line_along(
                msp,
                _lower_dim_start,
                lower_track_end,
                x_dir * -(pipe_hw + 35), f"{lower_total_len:.1f}")

    def _annotate_leg_angles(
            self, msp, leg_draw_data,
            upper_seg_positions, lower_seg_positions,
            x_dir, scale,
            _annotation_like_draw3, upper_start_elev):
        """脚架仰角標註：在每支脚架接合點繪製仰角。"""
        ...
        if n_legs == 1:
            ...
            if _annotation_like_draw3 and abs(upper_elev - upper_start_elev) > 1:
                upper_elev = upper_start_elev
    ...
    def _annotate_vert_gap(
        ...
        self._draw_dimension_line(
            msp,
            (start_ref_x, min(l_sy, u_sy)),
            (start_ref_x, max(l_sy, u_sy)),
            -x_dir * _vert_gap_offset, f"{_start_gap_val:.1f}",
            vertical=True)

        if abs(_end_gap_val - _start_gap_val) > 3:
            self._draw_dimension_line(
                msp,
                (ref_end_x, min(end_ly, end_uy)),
                (ref_end_x, max(end_ly, end_uy)),
                x_dir * _vert_gap_offset, f"{_end_gap_val:.1f}",
                vertical=True)

    def _annotate_inner_dist(
            self, msp,
            start_ref_x, u_sy, l_sy,
            ref_end_x, end_uy, end_ly,
            _start_gap_val, _end_gap_val,
    ...
    def _annotate_end_dist(
        ...
        def _get_track_angle_at_x(seg_positions, draw_x):
            """在 seg_positions 中找到 draw_x 所在段的仰角。"""
            for seg in seg_positions:
                sx, ex = seg[0], seg[2]
                if min(sx, ex) - 1 <= draw_x <= max(sx, ex) + 1:
                    return seg[5]
            best_a, best_d = 0, float('inf')
            for seg in seg_positions:
                d = abs((seg[0] + seg[2]) / 2 - draw_x)
                if d < best_d:
                    best_d, best_a = d, seg[5]
            return best_a
        ...
        if _end_inner_dist > 1:
            ...
            _eid_y1 = min(end_ly, end_uy) + pipe_hw
            _eid_y2 = max(end_ly, end_uy) - pipe_hw
            _eid_x_off = x_dir * 12
        ...
        if _start_inner_dist > 1:
            ...
            _si_y1 = min(l_sy, u_sy) + pipe_hw
            _si_y2 = max(l_sy, u_sy) - pipe_hw
            _si_x_off = -x_dir * 15
    ...
    def _annotate_leg_to_bend(
            self, msp, leg_draw_data,
            upper_seg_positions,
            pipe_hw, x_dir,
            _is_drawing1, _start_is_transition, _annotation_like_draw3):
        """脚架至最近彎曲點距標註（Rectangle 1）。"""
        bend_pts_upper = []
        cum = 0
        for i, seg in enumerate(upper_seg_positions):
            cum += seg[4]
            if i < len(upper_seg_positions) - 1:
                bend_pts_upper.append({
                    'x': seg[2], 'y': seg[3], 'cum': cum
                })
        upper_total_len = cum

        for ld in leg_draw_data:
            leg_cum = ld['t'] * upper_total_len
            best_bp, best_d = None, float('inf')
            for bp in bend_pts_upper:
    ...
    def generate_sub_assembly_drawing(self, output_dir: str = "output") -> List[str]:
        ...
        # Drawing 2: 彎軌施工圖 (Curved Track)
        # ================================================================
        _SHOW_BASIC_D2 = self._drafter_config.get('show_basic_info', False)
        _SHOW_TRACK_REL_D2 = self._drafter_config.get('show_track_relations', False)
        try:
            log_print("\n--- Drawing 2: 彎軌施工圖 ---")
            doc2 = ezdxf.new(dxfversion='R2010')
            msp2 = doc2.modelspace()
            # 載入中心線 linetype
            doc2.linetypes.add('CENTER', pattern=[0.0, 5.0, -2.0, 1.0, -2.0])
            ...
            _curved_rail_spacing = rail_spacing  # fallback
            if c_upper and c_lower:
                _u_fid_c = c_upper[0].get('solid_id', '')
                _l_fid_c = c_lower[0].get('solid_id', '')
                _u_cen = class_map_all.get(_u_fid_c, {}).get('centroid', None)
                _l_cen = class_map_all.get(_l_fid_c, {}).get('centroid', None)
                if _u_cen and _l_cen:
                    gn = self._ground_normal
                    y_up = (gn == (0, 1, 0))
                    _curved_rail_spacing = (abs(_u_cen[1] - _l_cen[1]) if y_up
                                            else abs(_u_cen[2] - _l_cen[2]))
                    log_print(f"  彎軌 rail_spacing={_curved_rail_spacing:.1f} "
                              f"(vs averaged={rail_spacing:.1f})")
            # 儲存到 stp_data 供 Drawing 3 使用
            stp_data['curved_rail_spacing'] = _curved_rail_spacing

            # 軌道兩端點 3D 距離標註（上圖下方）
            # 計算弧管端點間 3D 距離
            _arc_ep_3d_dist = 0
            if u_sp_3d != (0, 0, 0) and u_ep_3d != (0, 0, 0):
                _d3 = math.sqrt(sum((a - b)**2 for a, b in zip(u_sp_3d, u_ep_3d)))
                _arc_ep_3d_dist = _d3
            elif plan_valid:
                # fallback: XY chord + height
                _xy_ch = math.sqrt((plan_p1[0] - plan_p2[0])**2 + (plan_p1[1] - plan_p2[1])**2)
                _arc_ep_3d_dist = math.sqrt(_xy_ch**2 + arc_height_gain**2)

            if plan_valid:
                p1_d = _m2u(*plan_p1)
                p2_d = _m2u(*plan_p2)
                chord_dim_y = min(p1_d[1], p2_d[1]) - 5
                dim_text = f"{_arc_ep_3d_dist:.0f}" if _arc_ep_3d_dist > 0 else f"{chord_length:.0f}"
                if _SHOW_BASIC_D2:
            ...
            # Bounding box（分離計算：軌道用於 scale，含腳架用於 offset）
            leg_lengths = stp_data.get('leg_lengths', [])
            _max_below = 0
            _max_above = 0
            if leg_lengths:
                for _ll in leg_lengths:
                    _remain = max(0, _ll - rail_spacing)
                    _above = min(_remain * 0.08, 40)
                    _below = _remain - _above
                    _max_below = max(_max_below, _below)
                    _max_above = max(_max_above, _above)
            all_2d = u_pts_2d + l_pts_2d
            if all_2d:
                lb_x0 = min(p[0] for p in all_2d) - pipe_r
                lb_x1 = max(p[0] for p in all_2d) + pipe_r
                # 軌道自身的 Y 範圍
                track_y0 = min(p[1] for p in all_2d) - pipe_r
                track_y1 = max(p[1] for p in all_2d) + pipe_r
            else:
                lb_x0, lb_x1 = -200, 200
                track_y0, track_y1 = -200, 200

            # 含腳架延伸的完整 Y 範圍
            full_y0 = track_y0 - _max_below
            full_y1 = track_y1 + _max_above

            model_w_l = max(lb_x1 - lb_x0, 1)
            model_h_full = max(full_y1 - full_y0, 1)

            # 以完整內容（含腳架）計算 scale，提高填充率
            lower_scale = min(lower_area_w / model_w_l,
                              lower_area_h / model_h_full) * 0.88
            ...
            def _draw_pipe_curve(msp, pts_2d, hw, center_color=1):
                ...
                # 中心線（點劃線樣式）
                msp.add_lwpolyline(c_pts, dxfattribs={
                    'color': center_color, 'linetype': 'CENTER'})
                # 法線偏移計算管壁
                wall_u, wall_d = [], []
                ...
                if len(wall_u) >= 2:
                    ...
                    # 端部封口線
                    msp.add_line(wall_u[0], wall_d[0])
                    msp.add_line(wall_u[-1], wall_d[-1])
            ...
            # 繪製下軌（紅色中心線 + 黑色管壁）
            if l_pts_2d:
                _draw_pipe_curve(msp2, l_pts_2d, pipe_hw_l, center_color=1)

            # 支撐架（連接上下軌，不超過上下軌範圍 + X 標記 + 圓圈）
            if bracket_count > 0 and u_pts_2d and l_pts_2d:
                n_u = len(u_pts_2d)
                n_l = len(l_pts_2d)
                leg_hw = min(pipe_hw_l * 0.6, 1.5)  # 支撐架管壁半寬
                x_sz = min(1.5, pipe_hw_l * 0.6)     # X 標記大小
                circle_r = x_sz * 0.5
                for bi in range(bracket_count):
                    ...
                    # 支撐架管壁雙線（從上軌到下軌）
                    msp2.add_line((leg_top[0] - leg_hw, leg_top[1]),
                                 (leg_foot[0] - leg_hw, leg_foot[1]))
                    msp2.add_line((leg_top[0] + leg_hw, leg_top[1]),
                                 (leg_foot[0] + leg_hw, leg_foot[1]))
                    ...
                    _bdx = l_pt[0] - u_pt[0]
                    _bdy = l_pt[1] - u_pt[1]
                    _bd = math.sqrt(_bdx**2 + _bdy**2)
                    if _bd > 1e-9:
                        _bdx, _bdy = _bdx / _bd, _bdy / _bd
                    else:
                        _bdx, _bdy = 0, -1
                    _tdx, _tdy = -_bdy, _bdx
                    _xax = (_bdx + _tdx) * x_sz
                    ...
                    # X 標記（上軌穿越處）
                    msp2.add_line((u_pt[0] - _xax, u_pt[1] - _xay),
                                 (u_pt[0] + _xax, u_pt[1] + _xay))
                    msp2.add_line((u_pt[0] - _xbx, u_pt[1] - _xby),
                                 (u_pt[0] + _xbx, u_pt[1] + _xby))
                    # X 標記（下軌穿越處）
                    msp2.add_line((l_pt[0] - _xax, l_pt[1] - _xay),
                                 (l_pt[0] + _xax, l_pt[1] + _xay))
                    msp2.add_line((l_pt[0] - _xbx, l_pt[1] - _xby),
                                 (l_pt[0] + _xbx, l_pt[1] + _xby))
            ...
            # ================================================================
            # 球號標註
            # 軌道取料球號：U1（上軌）、D1（下軌）→ 藍色文字 + 引線指向材料中心
            # BOM 球號：1（支撐架）→ 業界慣例氣泡
            # ================================================================
            balloon_r = 5.0      # BOM 氣泡圓半徑
            balloon_th = 3.5     # BOM 球號文字高度

            # ---- 軌道取料球號：U1（上軌）— flag: show_basic_info ----
            if _SHOW_BASIC_D2 and u_pts_2d and len(u_pts_2d) > 10:
                u_mid_idx = len(u_pts_2d) // 2
                u_center = _m2l(*u_pts_2d[u_mid_idx])
                self._draw_cl_label(msp2, 'U1', u_center, dim_side=+1)

            # ---- 軌道取料球號：D1（下軌）— flag: show_basic_info ----
            if _SHOW_BASIC_D2 and l_pts_2d and len(l_pts_2d) > 10:
                l_mid_idx = len(l_pts_2d) // 2
                l_center = _m2l(*l_pts_2d[l_mid_idx])
                self._draw_cl_label(msp2, 'D1', l_center, dim_side=-1)

            # ---- BOM 球號：1（支撐架）— flag: show_basic_info ----
            if (_SHOW_BASIC_D2 and bracket_count > 0 and u_pts_2d and l_pts_2d
                    and len(u_pts_2d) > 5 and len(l_pts_2d) > 5):
                # 選中間支撐架
                mid_bi = bracket_count // 2
                n_u = len(u_pts_2d)
                n_l = len(l_pts_2d)
                idx_u_mid = int((mid_bi + 0.5) / bracket_count * (n_u - 1))
                idx_l_mid = int((mid_bi + 0.5) / bracket_count * (n_l - 1))
                idx_u_mid = max(0, min(idx_u_mid, n_u - 1))
                idx_l_mid = max(0, min(idx_l_mid, n_l - 1))
                brk_u = _m2l(*u_pts_2d[idx_u_mid])
                brk_l = _m2l(*l_pts_2d[idx_l_mid])
                # 支撐架中點
            ...
            log_print(f"  球號標註完成（U1, D1, BOM-1）")

            # ---- 高低差標註 — flag: show_track_relations ----
            _hg_dim_x = None
            _hg_lower_y = None
            _hg_lower_track_y = None
            if _SHOW_TRACK_REL_D2 and arc_height_gain > 1 and u_pts_2d:
                ...
                self._draw_dimension_line(msp2,
                                          ...
                                          vertical=True)
                _hg_dim_x = dim_x_right
                _hg_lower_y = min(u_start_d[1], u_end_d[1])
                if l_pts_2d:
                    if u_start_d[1] < u_end_d[1]:
                        _hg_lower_track_y = _m2l(*l_pts_2d[0])[1]
                    else:
                        _hg_lower_track_y = _m2l(*l_pts_2d[-1])[1]

            # ---- 上下軌距離標註 — flag: show_track_relations ----
            if _SHOW_TRACK_REL_D2 and _curved_rail_spacing > 0 and _hg_dim_x is not None and \
                    _hg_lower_y is not None and _hg_lower_track_y is not None:
                self._annotate_rail_spacing(
                    msp2,
                    (_hg_dim_x, _hg_lower_y),
                    (_hg_dim_x, _hg_lower_track_y),
                    10, _curved_rail_spacing)
            ...
            if _SHOW_BASIC_D2 and cl_items_for_d2:
            ...
            if _SHOW_BASIC_D2 and bom2_items:
                bom2_x = cl_table_x
